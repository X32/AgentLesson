import websockets, os, dashscope, asyncio, wave
...
async def save_data(message):
    ...
    if len(audio_buffer) >= 64 * 1024:
        ...
        time_stamp = datetime.now().strftime('%Y%m%d_%H%M%S_%f')   # 时间戳命名不重复
        temp_wav_name = f"{time_stamp}.wav"
        temp_wav_path = f"./testcode/audio/{temp_wav_name}"
...
# 定义WebSocket服务器和端口信息
async def main():
    server = await websockets.serve(ws_handle,"0.0.0.0",8765)
